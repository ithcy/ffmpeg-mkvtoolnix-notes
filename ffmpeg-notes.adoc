...
*Sidecar (aka external) subtitles* are standalone subtitle files in a format such as https://www.wikiwand.com/en/SubRip[SubRip^] (`+.srt+`) or https://www.wikiwand.com/en/DirectVobSub[VobSub^] (`+.idx/.sub+`) that are typically placed alongside, and usually share a base name with, the main video file. (i.e. `+my_video.mkv+` and `+my_video.eng.srt+`)
