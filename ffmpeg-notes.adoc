...
*Sidecar (aka external) subtitles* are standalone subtitle files in a format such as https://www.wikiwand.com/en/SubRip[SubRip] (`+.srt+`) or https://www.wikiwand.com/en/DirectVobSub[VobSub] (`+.idx+`/`+.sub+`) that are typically placed alongside the main video file and share a base filename with the video. (i.e. `+video.mkv+` and `+video.eng.srt+`)
====
...
NOTE: The subtitle is converted to `+mov_text+` format (aka Timed Text/TTXT, https://www.wikiwand.com/en/MPEG-4_Part_17[MPEG-4 Part 17]) since MP4 containers do not support SRT subtitles.
