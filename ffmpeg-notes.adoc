...
*Sidecar (aka external) subtitles* are standalone subtitle files in a format such as https://www.wikiwand.com/en/SubRip[SubRip,role=external,window=_blank] (`+.srt+`) or https://www.wikiwand.com/en/DirectVobSub[VobSub,role=external,window=_blank] (`+.idx+`/`+.sub+`) that are typically placed alongside the main video file and share a base filename with the video. (i.e. `+video.mkv+` and `+video.eng.srt+`)
