...
*Sidecar (aka external) subtitles* are standalone subtitle files in a format such as SubRip (`+.srt+`) or VobSub (`+.idx+`/`+.sub+`) that are typically placed alongside the main video file and share a base filename with the video. (i.e. `+video.mkv+` and `+video.eng.srt+`)
====
